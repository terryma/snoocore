...
function TestServer(port, responseStatus) {
  ...
  self.start = function () {
    ...
    var serverOptions = {
      ...
      key: fs.readFileSync(path.join(__dirname, '..', '..', '..', 'test', 'src', 'ssl', 'key.pem')),
      cert: fs.readFileSync(path.join(__dirname, '..', '..', '..', 'test', 'src', 'ssl', 'cert.pem'))
    };
    ...
    return when.promise(function (r) {
      ...
      return callbacks.call(ph.createPage).then(function (page) {
        ...
        var steps = [function () {
          ...
          var openPage = when.promise(function (r) {
            page.open(url, r);
          });
          ...
          // click allow or deny button in form
          ...
          // get the location href we have landed on
...
//# sourceMappingURL=../src/TestServer.js.map