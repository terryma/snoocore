...
describe(__filename, function () {
  ...
  it('application only oauth calling a user specific endpoint should fail', function () {
    ...
    return reddit('/api/v1/me').get().then(function (data) {
      ...
      return expect(error.message.indexOf('Must be authenticated with a user to make this call')).to.not.equal(-1);
    });
  });
        ...
        return reddit.auth(accessToken).then(function () {
          ...
          return _when2['default'].promise(function (resolve, reject) {
            ...
            reddit('/api/v1/me').get().then(function () {
              ...
              expect(error.message.indexOf('Access token has expired. ' + 'Listen for the "access_token_expired" event to ' + 'handle this gracefully in your app.')).to.not.equal(-1);
...
//# sourceMappingURL=../src/behavior-test.js.map